<!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
        <title>Title</title>
        <link rel="stylesheet" href="category.css">

          <?php
               include '../../Repeating-pages/topnav/topnav2-customer.php';

               /*------------viewing newly added category by admin------------------------*/
               $result = require '../../Admin/Adddeletecategory/add_delete_selection.php';
          ?>

          <link rel="stylesheet" href="../../Repeating-pages/topnav/topnav.css">

    
    </head>

    <body>
         <div class="headingcategory"> <h2>Categories</h2></div>
         
      <div class="box1">
        <div class="imageboxes">
          <div class="grid-container">

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item"> 
                         <img src="../../Images/category/plumber.png" alt="plumber" id="img">
                         <h4>Plumber</h4>
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/masons.png" alt="Masons" id="img">
                              <h4>Masons</h4>      
                    </div>
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/tile.png" alt="tile" id="img">
                              <h4>Tile</h4> 
                    </div>
               </a>  

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/saw.png" alt="carpenters" id="img">
                              <h4>Carpenters</h4>
                    </div>
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/paintbrush.png" alt="painters" id="img">
                              <h4>Painters</h4>
                    </div>
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/electrician.png" alt="electricians" id="img">
                              <h4>Electricians</h4>
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/planting.png" alt="landscape" id="img">
                              <h4>Landscape</h4>
                    </div>
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/worker.png" alt="worker" id="img">
                              <h4>Worker</h4>
                    </div>
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                         <img src="../../Images/category/businessman.png" alt="professionals" id="img">
                         <h4>Professionals</h4>
                    </div>
               </a>
  
               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/delivery-truck.png" alt="truck" id="img">
                              <h4>Truck</h4> 
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src= "../../Images/category/concrete-mixer-tool.png" alt="concrete slab" id="img">
                              <h4>Concrete slab</h4>
                    </div> 
               </a>


               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/television.png" alt="equipment repairing"  id="img">
                              <h4>Equipment<br> repairing</h4>
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/air-conditioner.png" alt="A/c"  id="img">
                              <h4>A/C</h4> 
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/welding.png" alt="welding" id="img">
                              <h4>Welding</h4>
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/whisk-and-bowl.png" alt="cooking" id="img">
                              <h4>Cooking</h4>  
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/armchair.png" alt="cushion" id="img">
                              <h4>Cushions</h4>
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/repairing-car.png" alt="repairs" id="img">
                              <h4> vehicle <br>Repairs</h4>
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/cleaning.png" alt="cleaners" id="img">
                              <h4>Cleaners</h4>
                    </div>
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/cctv.png" alt="cctv" id="img">
                              <h4>CCTV</h4>
                    </div>
               </a> 

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/ceiling.png" alt="ceiling" id="img">
                              <h4>Ceiling</h4>
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/water-well.png" alt="well" id="img">
                              <h4>Well</h4>
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/rocking-chair.png" alt="chair" id="img">
                              <h4>Chairs</h4>
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/rent-hanging-signal.png" alt="rent tools" id="img">
                              <h4>Rent-tools</h4> 
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/curtains.png" alt="curtains" id="img">
                              <p class=category>
                              <h4>Curtains</h4>
                    </div>
               </a> 

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/solar-panel.png" alt="solar" id="img">
                              <h4>Solar</h4>
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/demolition.png" alt="demo" id="img">
                              <h4>Demolition</h4>
                    </div>
               </a> 

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/insecticide.png" alt="pest"id="img">
                              <h4>Pest</h4>
                    </div>
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/tea-mug.png" alt="design" id="img">
                              <h4>Mug<br> designer</h4>
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/moving.png" alt="movers" id="img">
                              <h4>Movers</h4>
                    </div> 
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/recycle.png" alt="aluminium" id="img">
                              <h4>Aluminium</h4>
                    </div>
               </a>

               <a href="../CategoryWorkerPg/CategoryWorker.php">
                    <div class="grid-item">
                              <img src="../../Images/category/waiter.png" alt="waiter" id="img">
                              <h4>Catering</h4>
                    </div> 
               </a>

               <!--------------------Viewing results -categories-------------------->
                <?php
                    $text = "";
                    if (mysqli_num_rows($result) > 0) {   
                         $a=1;       
                         while ($images = mysqli_fetch_assoc($result)) {
                              $text .= "<a href='../CategoryWorkerPg/CategoryWorker.php'>";
                              $text .= "<div class='grid-item2'>";
                              $text .= "<img src='../../Admin/Adddeletecategory/uploads/".$images['file_url']."' id='img2".$a."'>";
                              $text .= "</div></a>";
                              $a++;
                         }
                    }
                    echo $text;
                ?>
              
          </div>
        </div>
      </div>



   </body>


</html>