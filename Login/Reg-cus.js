...
form.addEventListener('submit', (e) => {
  ...
  if (inputs[1].value!="" && inputs[1].value.match(contactType)==null) {
    ...
    inputs[1].placeholder="Please enter a valid contact number";
    validity=false;
  }

  //validate email address
  let emailType = /^([a-z\d\.-]+)@([a-z\d-]+)\.([a-z]{2,8})(\.[a-z]{2,8})?$/;
  if (inputs[2].value!="" && inputs[2].value.match(emailType)==null) {
    inputs[2].value="";
    inputs[2].style.border = "3px solid rgb(228, 29, 22)";
    inputs[2].placeholder="Email is not valid";
    validity=false;
  }
  ...
  if (inputs[3].value!="" && inputs[3].value.match(passwordType)==null) {
    inputs[3].value="";
    inputs[3].style.border = "3px solid rgb(228, 29, 22)";
    inputs[3].placeholder="Please enter a valid password";
    validity=false;
  }
  ...
  if (inputs[3].value != inputs[4].value) {
    ...
    inputs[4].placeholder="Passwords do not match";
    validity=false;
}
